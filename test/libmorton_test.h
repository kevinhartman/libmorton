...
#include "../morton-nd/morton-nd/include/mortonND_BMI2.h"
...
uint64_t control_encode(size_t bitCount, const std::valarray<uint64_t>& fields) {
	...
	return (control_encode<FieldCount>(bitCount, fields[std::slice(1, fields.size() - 1, 1)]) << 1)
 ...
 */
template<typename...T>
uint64_t control_encode(size_t bitCount, T... fields) {
	return control_encode<sizeof...(fields)>(bitCount, { fields... });
}
